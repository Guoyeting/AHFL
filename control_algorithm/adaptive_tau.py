...
class ControlAlgAdaptiveTauServer:
    ...
    def compute_new_tau(self, data_size_local_all, data_size_total, it_each_local, it_each_global, max_time,
                        step_size, tau, use_min_loss, participant):
        n_nodes_tmp = len(participant)
        beta_adapt = 0
        delta_adapt = 0
        rho_adapt = 0
        ...
        for n in range(n_nodes_tmp):
            msg = recv_msg(self.client_sock_all[participant[n]], 'MSG_CONTROL_PARAM_COMPUTED_CLIENT_TO_SERVER')
            ...
            if control_param_computed_this_client:
                ...
                msg = recv_msg(self.client_sock_all[participant[n]], 'MSG_BETA_RHO_GRAD_CLIENT_TO_SERVER')
