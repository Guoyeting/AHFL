...
class ModelCNNAbstract(abc.ABC):
    ...
    def _optimizer_op_init(self, learning_rate=None):
    ...
    def get_init_former_weight(self, dim, rand_seed=None):
        if not self.graph_created:
            raise Exception('Graph is not created. Call create_graph() first.')
        ...
        if rand_seed is not None:
            ...
            tf.reset_default_graph()
            tf.set_random_seed(rand_seed)
            self.create_former_graph(learning_rate=self.learning_rate, split_point= self.split_point)  # This creates the session as well
    ...
    def get_init_latter_weight(self, dim, rand_seed=None):
        if not self.graph_created:
            raise Exception('Graph is not created. Call create_graph() first.')
        ...
        if rand_seed is not None:
            ...
            tf.reset_default_graph()
            tf.set_random_seed(rand_seed)
            self.create_latter_graph(learning_rate=self.learning_rate, split_point= self.split_point)  # This creates the session as well
    ...
    def middle_output(self, imgs, w, sample_indices):
        if not self.graph_created:
            raise Exception('Graph is not created. Call create_graph() first.')

        self.assign_flattened_weight(self.session, w)
        if self.split_point == 1:
            output = self.session.run(self.h_norm1, feed_dict={self.x: [imgs[i] for i in sample_indices]})
        elif self.split_point == 2:
            output = self.session.run(self.h_pool2_flat, feed_dict={self.x: [imgs[i] for i in sample_indices]})
        elif self.split_point == 3:
            output = self.session.run(self.h_fc1, feed_dict={self.x: [imgs[i] for i in sample_indices]})

        return output

    def latter_gradient(self, mid_output, w, labels):
        if not self.graph_created:
            raise Exception('Graph is not created. Call create_graph() first.')
        ...
        if self.split_point == 1:
            grad_var_list = self.session.run(self.grad, feed_dict={self.h_norm1: mid_output, self.y_: labels})#[labels[i] for i in sample_indices]})
        elif self.split_point == 2:
            grad_var_list = self.session.run(self.grad, feed_dict={self.h_pool2_flat: mid_output, self.y_: labels})#[labels[i] for i in sample_indices]})
        elif self.split_point == 3:
            grad_var_list = self.session.run(self.grad, feed_dict={self.h_fc1: mid_output, self.y_: labels})#[labels[i] for i in sample_indices]})

        grad_flatten_list = []
        for l in grad_var_list[:-1]:
            grad_flatten_list.append(np.reshape(l[0], l[0].size))

        grad_flatten_array = np.hstack(grad_flatten_list)

        mid_output_grad = grad_var_list[-1][0]
        del grad_var_list
        del grad_flatten_list

        return grad_flatten_array, mid_output_grad

    def former_gradient(self, imgs, w, sample_indices, mid_output_grad):
        grad_var_list = self.session.run(self.grad, feed_dict={self.x: [imgs[i] for i in sample_indices], self.mid_grad: mid_output_grad})

        grad_flatten_list = []
        for l in grad_var_list:
            grad_flatten_list.append(np.reshape(l[0], l[0].size))
            #grad_flatten_list.append(np.reshape(l, l.size))
