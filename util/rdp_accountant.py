...
'''
sample_ratio_local = 100/60000
sigma_local = 0.2
orders = [1 + x / 10. for x in range(1, 100)] + list(range(12, 64))
delta_local = 1e-2
epsilon_local = 30
for sigma_local in [0.2, 0.4, 0.6, 0.8, 1]:
  for epoch in range(10000):
    rdp = compute_rdp(q=sample_ratio_local, noise_multiplier=sigma_local,
                                                  steps=int(epoch / sample_ratio_local), orders=orders)
    if get_privacy_spent(orders, rdp, target_delta=delta_local)[0] > epsilon_local:
      print(sigma_local, epoch)
      break
'''